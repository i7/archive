...
To say sub-agenda for (act - an attempt) indenting (indentation level - a number) times:
	repeat with item running through all attempts which fulfill the act:
		...
		say "[item] ([if the item is hindered]hindered by the [meddling check rule of the item][else if the item is partly successful]solved the [meddling check rule of the item][else if the item is moot]moot[else][attempt condition of the item][end if])[roman type][line break][sub-agenda for the item indenting (the indentation level + 1) times]"
...
When hindered by the convert remove to take rule, pretend it worked.
When hindered by the convert put to drop where possible rule, pretend it worked.
When hindered by the convert insert to drop where possible rule, pretend it worked.
When hindered by the convert enter door into go rule, pretend it worked.
When hindered by the convert enter compass direction into go rule, pretend it worked.
When hindered by the convert exit into go out rule, pretend it worked.
When hindered by the convert exit into get off rule, pretend it worked.
When hindered by the convert show to yourself to examine rule, pretend it worked.
When hindered by the translate asking for to giving rule, pretend it worked.
When hindered by the standard pushing in directions rule, pretend it worked.
When hindered by the determine map connection rule, pretend it worked.
...
When hindered by the carrying requirements rule: we could try taking the noun.
...
When hindered by the can't take people's possessions rule: we could try asking the holder of the noun for the noun.[
