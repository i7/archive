Version 1/131010 of Conversation Builder (for glulx only) by Chris Conley begins here. 
...
Instead of asking about (this is the vague asking rule):
	if the current interlocutor is nothing, say "You need to talk to someone first." instead;
	now the variable snippet is the topic understood; 
	store base quip text;
	say "That question is not implemented. Draft a new one? >";
	if the player consents:
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "[sample-quip] is a questioning quip. "; 
		close file "NewConversation" for writing; 
		if we need to escape trouble words is true, escape troubled quip-names;
		carry out the filling in standard quip activity.
...
Instead of telling about  (this is the vague telling rule):
	...
	now the variable snippet is the topic understood; 
	store base quip text;
	say "That remark is not implemented. Draft a new one? >";
	if the player consents:
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "[sample-quip] is an informative quip. "; 
		close file "NewConversation" for writing; 
		if we need to escape trouble words is true, escape troubled quip-names;
		carry out the filling in standard quip activity.
...
A quip-building rule (this is the getting a main comment rule):
	say "What would you like the player to say at this point?[paragraph break]";
...
A quip-building rule (this is the setting mentions rule):
	move quip line forward;
	say "[line break]Based on what you just wrote, what subjects does this remark refer to? ";
	if the suppressed subject list option is active, do nothing;
	otherwise say "[if the number of subjects is 0]There are no conversation subjects yet defined[otherwise]Current conversation subjects include [italic type][list of subjects][roman type][end if][one of], though quips may also refer to objects in the game[or][stopping]. ";
	say "[first time](Provide your answer as a list separated by commas. It is not necessary to list synonyms for the same subject or game-object, as they will be included in parsing automatically.)[only][line break]";
	write "	It mentions " after file "NewConversation";
	stuff buffer;
	end new quip line.

A quip-building rule (this is the getting a response rule):
	say "[line break]And what would you like [the current interlocutor] to say in response? [paragraph break]"; 
	write "	The response is " after file "NewConversation";
	get text for a quoted field;
	end new quip line.

A quip-building rule (this is the making interlocutor specific rule):
	say "[line break]Is this something that the player can say only to [the current interlocutor]? >";
	if the player consents:
...
A quip-building rule (this is the marking repeatable quips rule):
	say "May the player say this more than once[if the single character setting is q-set-no] to the same character[end if]? >";
	if the player consents:
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "	It is repeatable";
		close file "NewConversation" for writing;
		end new quip line.

A quip-building rule (this is the marking restrictive quips rule):
	say "Is the player going to be restricted to a small choice of responses to this? >";
	if the player consents:
		now the currently open reference is the reference after opening file "NewConversation" for writing;
...
A quip-building rule (this is the making remark follow the last quip rule):
	...
	say "Does this remark have to follow immediately after [the current quip]? [if the current quip is restrictive](Because the current quip restricts the player to a limited set of answers, you should choose YES if you want the player to be able to use this message as an answer to the foregoing remark.)[end if] >";
	if the player consents:
		now immediacy setting is q-set-yes;
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "	It directly-follows [the true-name of the current quip]";
		close file "NewConversation" for writing;
		end new quip line.
...
A quip-building rule (this is the making remark distantly follow the last quip rule):
	...
	say "Does this remark have to come [italic type]some time[roman type] (but maybe not immediately) after [the current quip]? >";
	if the player consents:
		now the indirection setting is q-set-yes;
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "	It indirectly-follows [the true-name of the current quip]";
		close file "NewConversation" for writing;
		end new quip line;.
...
A quip-building rule (this is the making remark follow the previously defined quip rule):
	...
	say "Does this remark have to come immediately after the quip you just added, namely [the last-prettified-quip]? >";
	if the player consents:
		now immediacy setting is q-set-yes;
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "	It directly-follows [the last-designed-quip]";
		close file "NewConversation" for writing;
		end new quip line;.
...
A quip-building rule (this is the making remark distantly follow the previously defined quip rule):
	...
	say "Does this remark have to come [italic type]some time[roman type] (but maybe not immediately) after the quip you just added, namely [the last-prettified-quip]? >";
	if the player consents:
		now the currently open reference is the reference after opening file "NewConversation" for writing;
		say "	It indirectly-follows [the last-designed-quip]";
		close file "NewConversation" for writing;
		end new quip line.
...
Carry out looking at new source (this is the source examination rule):
	say "You review your new code: [paragraph break]"; 
...
We begin by downloading and installing the Threaded Conversation extension from the Inform website, if we don't already have it installed. Conversation Builder depends on this. (And Conversation Framework by Eric Eve. Threaded Conversation depends on that extension, as well.)
...
Section: Writing comment and response properties

Now CB will ask for a comment and response appropriate to this quip. Because pressing return will indicate the end of a comment or response, we should use the "[paragraph break]" substitution if we want to introduce additional lines of spacing into the middle of one of these fields. A maximum length of 3000 characters is set for comments and responses, as determined by the LARGE_BUFFER_LENGTH constant.
...
Comments and responses may safely be written with either single or double quotes; all double-quotes will be converted back to a single quote before being saved in the source.
...
Conversation Builder does not attempt to handle very complicated situations: there are many cases where we will want to write our own custom plausibility and availability rules, for instance. For any of the following tasks, we should consult the documentation for Threaded Conversation:
	...
	(5) making characters provide atypical responses to a whole set of quips under special circumstances (e.g., having a character who gets angry and responds rudely everything the player says) 
...
One way to do this would be to add the following bit of code to the source:
	...
	The cut off quips rule is listed before the getting a response rule in the quip-building rules.
