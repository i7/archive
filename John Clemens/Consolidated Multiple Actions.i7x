Consolidated Multiple Actions (for Glulx only) by John Clemens begins here.
...
[ GENERATE_ACTION_R i j k l; 
	...
	if (inp2 == 1) { if (inp1 == 1) second = special_number2; else second = special_number1; } 
	else second = inp2; 

	if (multiflag) { 
		if (multiple_object-->0 == 0) { L__M(##Miscellany, 2); return; } 
		if (toomany_flag) { toomany_flag = false; L__M(##Miscellany, 1); } 
		ProcessRulebook( (+ multiple action processing rulebook +) ); ! new in CMA
...
Last action consolidation results rule (this is the standard action consolidation results rule):
	...
	if the action_success_flag is true:
		...
		unless using the strict action consolidation checking option:
...
Last for reporting consolidated actions rule when asking someone to try dropping (this is the standard consolidated report dropping by others rule): say "[The actor] puts down [consolidated objects]."
...
Last for reporting consolidated actions rule when asking someone to try removing from (this is the standard consolidated report removing from by others rule): say "[The actor] picks up [consolidated objects]."
...
Last for reporting consolidated actions rule when asking someone to try putting on (this is the standard consolidated report putting on by others rule): say "[The actor] puts [consolidated objects] on [the second noun]."
...
Last for reporting consolidated actions rule when asking someone to try inserting into (this is the standard consolidated report inserting into by others rule): say "[The actor] puts [consolidated objects] into [the second noun]."
	...
	*: "Orderly Announcement"
	...
	After reading a command: 
		repeat with group running through collections:
			...
			while N matches the regular expression "[group]", replace the regular expression "(.*)[group](.*)" in N with "\1[Y]\2"; 
			change the text of the player's command to N.
...
Since the action only fails in one way, we can also add a new multiple action processing rule to consolidate the failed action reports as well.
...
And just for fun, we vary the printing of the consolidated objects list a bit.
	...
	*: "Burn, Baby, Burn"
	...
	Rule for supplying a missing second noun while burning with: change the second noun to the lighter.
