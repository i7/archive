...
Every turn when the current node is not the null-node (this is the node continuation rule):
  if conversed-this-turn is false, consider the node-continuation rules for the current node;
...
Before giving something to someone (this is the note giving rule):
  now conversed-this-turn is true.

Before showing something to someone (this is the note showing rule):
  now conversed-this-turn is true.
