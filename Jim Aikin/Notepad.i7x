...
Check an actor reading (this is the ordinary check reading rule):
	...
	otherwise:
		if the noun is not a notepad:
			say "[The actor] is unable to do that.";
			rule succeeds;
		otherwise if the memo of the noun is "":
			say "'But [the noun] is blank,' [the actor] protests.";
			rule succeeds.
...
Carry out an actor reading (this is the ordinary carry out reading rule):
	if the actor is the player:
		if the memo of the noun is not "":
			...
			say "On [the noun] is written [run paragraph on]";
	...
	otherwise:
		if the memo of the noun is not "":
			say "'On [the noun] is written ['][memo of the noun]['],' [the actor] reports.";
		otherwise:
			say "'Nothing is written on [the noun],' [the actor] reports."
...
After examining a notepad (called N) (this is the notepad output rule):
	if the memo of N is not "":
		...
		say "On [the N] is written [run paragraph on]";
		if term is terminated:
...
Check an actor writing on something (this is the ordinary check writing it on rule):
	...
	if the actor is the player:
		...
		otherwise if the second noun is non-edit-allowing:
			say "[The second noun] [if the second noun is plural-named]are[otherwise]is[end if] currently write-protected, and can't be written on." instead;
	...
	otherwise:
		if the second noun is not a notepad:
			say "[The actor] is unable to write anything on [the second noun].";
			rule succeeds;
		otherwise if the second noun is non-edit-allowing:
...
Carry out an actor writing on something (this is the ordinary carry out writing it on rule):
	...
	otherwise:
		say "[The actor] writes '[T]' on [the second noun]."
...
Check an actor adding to something (this is the ordinary check adding it to rule):
	...
	if the second noun is not a notepad:
		if the actor is the player:
			say "You can't write anything on [the second noun]." instead;
		otherwise:
			say "[The actor] can't write anything on [the second noun].";
			rule succeeds;
	otherwise if the second noun is non-edit-allowing:
		say "[The second noun] [if the second noun is plural-named]are[otherwise]is[end if] currently write-protected, and can't be written on.";
		rule succeeds;
...
Carry out an actor adding to something (this is the ordinary carry out adding it to rule):
	...
	otherwise:
		say "[The actor] adds [run paragraph on]";
	say "to the text on [the second noun]. The text now reads [run paragraph on]";
...
Check an actor copying something to something (this is the ordinary check copying it to rule):
	...
	otherwise if the second noun is not a notepad:
		...
		otherwise:
			say "[The actor] [run paragraph on]";
		say "can't copy text onto [the second noun]!";
		rule succeeds;
	otherwise if the noun is the second noun:
		...
		otherwise:
			say "[The actor] [run paragraph on]";
		say "can't copy something to itself.";
		rule succeeds;
	otherwise if the second noun is non-edit-allowing:
		say "[The second noun] [if the second noun is plural-named]are[otherwise]is[end if] currently write-protected, and can't be written on.";
		rule succeeds;
	...
	if carrying-pen is false:
		if the actor is the player:
			say "You have [run paragraph on]";
		otherwise:
			say "[The actor] has [run paragraph on]";
		say "nothing to write on [the second noun] with.";
		rule succeeds.
...
Carry out an actor copying something to something (this is the ordinary carry out copying it to rule):
	...
	otherwise:
		say "[The actor] copies [run paragraph on]";
	say "the contents of [the noun] to [the second noun], so that [the second noun] now reads [run paragraph on]";
	let term be the terminor of the memo of the second noun;
	if term is terminated:
		say "'[memo of the second noun]'[paragraph break]";
	otherwise:
...
Check an actor erasing (this is the ordinary check erasing rule):
	if the noun is not a notepad:
		say "There's nothing on [the noun] to erase.";
		rule succeeds;
	otherwise if the memo of the noun is "":
		say "At the moment, nothing is written on [the noun].";
		rule succeeds;
	otherwise if the noun is non-edit-allowing:
		say "[The noun] [if the noun is plural-named]are[otherwise]is[end if] currently write-protected, and can't be written on.";
		rule succeeds.
...
Carry out an actor erasing (this is the ordinary carry out erasing rule):
	...
	otherwise:
		say "[The actor] erases [run paragraph on]";
	say "what was written on [the noun], leaving it blank."
...
Check an actor erasing from something (this is the ordinary check erasing it from rule):
	if the second noun is not a notepad:
		...
		otherwise:
			say "[The actor] [run paragraph on]";
		say "can't erase anything from [the second noun].";
		rule succeeds;
	otherwise if the memo of the second noun is "":
		say "Nothing is written on [the second noun], so nothing can be erased.";
		rule succeeds;
	otherwise if the second noun is non-edit-allowing:
		say "[The second noun] [if the second noun is plural-named]are[otherwise]is[end if] currently write-protected, and can't be written on.";
		rule succeeds.
...
Carry out an actor erasing from something (this is the ordinary carry out erasing it from rule):
	...
	otherwise:
		if the actor is the player:
			say "The text '[T]' seems not to be written anywhere on [the second noun].";
		otherwise:
			say "'The text ['][T]['] seems not to be written anywhere on [the second noun],' [the actor] reports."
...
To use Notepad, first create a writing implement and a notepad. The writing implement is not a special kind of thing; anything can be a writing implement (for instance, a fountain pen, which might be implemented as a container). Add the writing implement to the list of allowed-pens of the notepad:
...
Any number of writing implements can be included in the allowed-pens list for a notepad. If you'd rather let the player write on the notepad without bothering with a writing implement (perhaps because the notepad object is a hand-held computer with a QWERTY keyboard), forget about the allowed-pens list. Instead, set the pen-needed property of the notepad to false:
 ...
 The text of a notepad (indexed text, in point of fact) is held in a property called memo. If you want the notepad to start the game with a message already written on it, put something in the memo property:
...
Notepad requests that the compiler set the maximum indexed text length to 2000. Overrunning this limit at run-time may cause an interpreter to crash. If you're concerned about this factor, you can increase the figure by putting a line like this in your source after including Notepad:

	Use maximum indexed text length of at least 5000.

In practice, indexed texts of less than 2000 characters may slow the interpreter noticeably. The erasing it from action can become quite sluggish with long texts. 
...
The player can "erase <any text> from <notepad>". ("delete" works as a synonym for "erase"). This command searches the entire text and removes all instances of <any text> from it. The erasing it from command is case-insensitive, so it will delete capital letters (if any have been added to a memo text by the author).
...
Due to the sheer number of outputs needed and their close linkages to indexed text, no attempt has been made to extract the output text into a table for ease of customization or use in other languages. To change the output, you'll need to copy the action rules into your source, give your copies new names, and then use a command such as "the new check erasing rule is listed instead of the ordinary check erasing rule in the check erasing rulebook."

All text is converted to lower-case when being entered. (Actually, by the parser before the write command is executed.) So it's not possible to capitalize words that are written.
...
Example: * Memorabilia - Some objects the player can write in.
	...
	*: "Memorabilia"
	...
	The player carries a fish. The description of the fish is "The fish is remarkably scaly."
	...
	The foggy mirror is in the Lab. The description is "The mirror is thickly coated with condensed steam." The foggy mirror is a notepad. The pen-needed of the foggy mirror is false. The memo of the foggy mirror is "Beware! Ghouls!"
