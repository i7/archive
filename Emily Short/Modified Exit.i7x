Version 5 of Modified Exit by Emily Short begins here.

"Changes the handling of the EXIT action, allowing commands such as EXIT PLATFORM and GET OUT OF CHAIR, making characters leave enterable objects before traveling, and altering the default interpretation of >OUT when the player is neither inside an object nor in a room with an outside exit."
...
Instead of going nowhere when the holder of the player is an enterable thing (called the divan) (this is the no motion without purpose rule): 
	...
	say "You couldn't go that way even if you weren't [if the divan is a supporter]on[otherwise]in[end if] [the divan]."
...
This is the implicit exiting rule:
	...
	if the person asked is the player and the sequential action option is not active:
		say "(first getting [if the chaise is a supporter]off[otherwise]out of[end if] [the chaise])[command clarification break]";
...
Check an actor standing (this is the check holder for standing rule):
	...
	begin;
		if the actor is the player, say "You're already up.";
		stop the action;
	end if.
