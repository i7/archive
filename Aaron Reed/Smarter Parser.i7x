...
To decide whether sp_regex (words - an indexed text) starts with (regex - an indexed text): (- SP_Starts_With_Regex({-pointer-to:words}, {-pointer-to:regex}) -).
...
[ SP_Starts_With_Regex wd regex    lenW lenR i c wordCtr thisWordMatch lastChar;
	...
	lenW = IT_CharacterLength(wd);
	lenR = IT_CharacterLength(regex);
	wordCtr = 0;
	thisWordMatch = true;
...
Rule for printing a parser error when the latest parser error is the I beg your pardon error (this is the do something useful with blank lines fallback rule):
	identify error as do something useful with blank lines fallback rule;
...
To decide whether contents of (str - indexed text) are just spaces: (- IsOnlySpaces({-pointer-to:str})  -).

To decide what indexed text is strip_special_characters for (cmd - indexed text): (- StripSpecialCharacters({-pointer-to:cmd}) -).
