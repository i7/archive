Version 9 of Scheduled Activities by John Clemens begins here.
...
Please report any bugs/suggestions to me at jdc20@psu.edu, or post them to rec.arts.int-fiction. ]
...
To mark (E - activity on objects) activities:
	repeat through the Table of Scheduled Events:
		if the SA_Object_Event entry is E, change the SA_Similar entry to 1.
...
Section 4 - Alternate scheduling phrases - unindexed

To schedule (E - activity on objects) for (T - object) in (n - number) minute: 
	schedule E for T in n minutes.

To schedule (E - activity) in (n - number) minute:
	schedule E in n minutes.

To schedule (E - activity on objects) for (T - object) in (d - number) turn:
	schedule E for T in d turns.

To schedule (E - activity) in (d - number) turn:
	schedule E in d minutes.

To schedule (E - activity on objects) for (T - object) in (d - number) turns from now:
	schedule E for T in d turns.

To schedule (E - activity on objects) for (T - object) in (d - number) turn from now:
	schedule E for T in d turns.

To schedule (E - activity) in (d - number) turns from now:
	schedule E in d minutes.

To schedule (E - activity) in (d - number) turn from now:
	schedule E in d minutes.

To schedule (E - activity on objects) in (n - number) minute for (C - description of objects): 
	repeat with T running through C begin; schedule E for T in n minutes; end repeat.

To schedule (E - activity on objects) in (d - number) turn for (C - description of objects):
	repeat with T running through C begin; schedule E for T in d turns; end repeat.

To schedule (E - activity on objects) in (d - number) turns from now for (C - description of objects):
	repeat with T running through C begin; schedule E for T in d turns; end repeat.

To schedule (E - activity on objects) in (d - number) turn from now for (C - description of objects):
	repeat with T running through C begin; schedule E for T in d turns; end repeat.
...
The change log and contact information are at the top of the extension code.
...
(note that the column names have been changed from previous versions).
...
Example: ** Bunnies - Grouping activity reports.
	...
	*:"Bunnies"
	...
	Carry out lobbing it toward:
		...
		change the target room to the room second noun from location;
		if the second noun is down, change the target room to location;
