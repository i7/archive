...
Object keyword highlighting is a truth state that varies. Object keyword highlighting is usually true. [Global activation of object keywords.]
...
Direction keyword highlighting is a truth state that varies. Direction keyword highlighting is usually true.
...
Topic keyword highlighting is a truth state that varies. Topic keyword highlighting is usually false.
...
Parser highlighting is a truth state that varies. Parser highlighting is usually false.
...
Carry out thing listing (this is the Keyword Interface carry out thing listing rule):
	show KI message for show-nearby-things.
...
[Now define the actual messages.]
...
keyword-instructions-object	"[Emphasiz]ed [o]object[x] keywords currently look like [o]this[x]."
keyword-instructions-direction	"[Emphasiz]ed [d]exit[x] keywords currently look like [d]this[x]."
keyword-instructions-topic	"[Emphasiz]ed [t]topic[x] keywords currently look like [t]this[x]."
keyword-instructions-parser	"[as the parser]Messages from the parser currently display like this[as normal]."
keyword-instructions-disabled	"**A warning: The [emphasiz]ed keywords are integral to [story title]'s design. It may be difficult or impossible to know how to continue if emphasis is not visible.**"
keyword-instructions-distinct	"**A warning: it may be useful to distinguish one type of keyword from another.**"
keyword-instructions-end	"Press 0 when you're finished here, or 9 to [if screen reader mode is true]de[end if]activate screen reader mode.[run paragraph on]"
screen-reader-activated	"Screen reader mode has been activated. [story title] uses [emphasiz]ed keywords to indicate important words you may type to advance the story. Your reader software may put a [recogniz]able vocal stress on [emphasiz]ed words like [o]this[x] one. If it doesn't, you can type [o]things[x] from any prompt for a list of nearby objects, or type [d]exits[x] for a list of nearby directions. Type [o]keywords[x] to adjust the style of keywords, or type screen reader with no space to toggle this mode on or off."
screen-reader-deactivated	"Screen reader mode has been deactivated."
no-available-exits	"It would seem there is nowhere to go."
show-available-exits	"From here, [if the number of viable directions is 1]the only way out is to[otherwise]you can go to[end if] [a list of viable directions]."
show-nearby-things	"Nearby [is-are a list of visible other things which are not carried by the player][if the number of visible other things which are not carried by the player is 0] of note[end if]."
...
Keyword Interface allows an emphasis style to be set for one of four possible categories. Each of these can be enabled or disabled independently by the author or player. By default, they are set as follows:
	...
	Object keyword highlighting is usually true.
	Direction keyword highlighting is usually true.
	Topic keyword highlighting is usually false.
	Parser highlighting is usually false.
...
When object keyword highlighting is set to true, the extension will automatically emphasize non-scenery objects listed by the parser. Objects may also be manually emphasized in any text by wrapping them in [o] and [x] substitutions.
...
We implement an extremely simple conversation system to demonstrate emphasized topic words. Note that a real conversation system would need to be much more robust and elaborate. We'll also create a variation on the exits verb to show our compassless exits.
	...
	*: "The Gnome's Holiday"
	...
	A speakable is a kind of thing. A speakable has some text called the response. Instead of doing anything other than keyword-asking to a speakable: say "That's merely a topic of conversation.". Does the player mean doing something other than keyword-asking to a speakable: it is very unlikely.  
	...
	Understand "about" as requesting the about text. Requesting the about text is an action out of world applying to nothing. Carry out requesting the about text: show KI message for keyword-introduction.  
