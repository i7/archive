Version 9 of Editable Stored Actions by Ron Newcomb begins here.
...
To decide whether the request part of (act - stored action):
	(- (STORED_ACTION_TY_Part({-pointer-to:act}, 4))  -).
...
To decide what indexed text is text part of (act - stored action):
	(- (STORED_ACTION_TY_Part({-pointer-to:act}, 5))  -).

To decide what indexed text is the participle part of (sa - stored action):
	decide on the participle part of (action name part of sa).

To decide what indexed text is the participle part of (act - an action name):
	let X be indexed text;
	let X be "[act]";
	replace the regular expression "\s+it\b.*$" in X with "";
	decide on X.

To decide what indexed text is the preposition part of (sa - stored action):
	decide on the preposition part of (action name part of sa).

To decide what indexed text is the preposition part of (act - an action name):
	let X be indexed text;
	let X be "[act]";
...
[  [ broken ]
To change/now the/-- text part of (act - a stored action) to/is (itxt - indexed text):
(- (STORED_ACTION_TY_SetPart({-pointer-to:act}, 5, INDEXED_TEXT_TY_Create({itxt}))); -).
]
...
[Chapter 7 - Alternate Phrasing with the Person First

To (pop - a person) is/to the/an/a/-- new/now/-- the/an/a/-- actor part/-- for/of (act - a stored action):
	(- (STORED_ACTION_TY_SetPart({-pointer-to:act}, 3, {pop})); -).

To (obj - an object) is/to the/an/a/-- new/now/-- the/an/a/-- noun part/-- for/of (act - a stored action):
	(- (STORED_ACTION_TY_SetPart({-pointer-to:act}, 1, {obj})); -).

To (obj - an object) is/to the/an/a/-- new/now/-- the/an/a/-- second noun part/-- for/of (act - a stored action):
	(- (STORED_ACTION_TY_SetPart({-pointer-to:act}, 2, {obj})); -).]



[ THE FOLLOWING ONLY WORK FOR 5Z71 OR EARLIER. THE CURRENT VERSION MUST USE INDEXED TEXT AND REGEXES. ]

[
Chapter 7 - participle part

To decide what text is participle part/-- for/of (act - a stored action): 
	decide on the participle part of the action name part of the act.

To decide what text is participle part/-- for/of (verb - an action name): 
	(- (ParticipleForActionName({verb})) -).

Include (-
[ ParticipleForActionName ac;
	ac = FindAction(ac);
	if (ac == 0) return EMPTY_TEXT_VALUE;
	ac = ActionData-->(ac+AD_NAME_BASE);
	if (ac ~= NULL) return ac;
	return EMPTY_TEXT_VALUE;
];
-).

Chapter 8 - preposition part

To decide what text is preposition part/-- for/of (act - a stored action): 
	decide on the preposition part of the action name part of the act.

To decide what text is preposition part/-- for/of (verb - an action name): 
	(- (PrepositionForActionName({verb})) -).

Include (-
[ PrepositionForActionName ac;
	ac = FindAction(ac);
	if (ac == 0) return EMPTY_TEXT_VALUE;
	ac = ActionData-->(ac+AD_NAME_BASE+2); 
	if (ac ~= NULL) return ac;
	return EMPTY_TEXT_VALUE;
];
-).
