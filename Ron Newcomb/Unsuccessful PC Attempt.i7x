Unsuccessful PC Attempt by Ron Newcomb begins here.
...
This is the modified check stage rule: 
	anonymously consider the specific check rulebook;
	...
	if the rule failed begin;
		if the actor is the player, consider the unsuccessful attempt by rules; [ ..will be called for NPCs from the usual place]
		rule fails;
	end if.

To anonymously consider (R - a rule): (- anon_consider({R}); -).

Include (-
[ anon_consider R;
	@push untouchable_silence; 
	untouchable_silence = (+ silent checks +);       ! checked by L__M() and related library functions
	if (reason_the_action_failed = ProcessRulebook(R))    ! this sets, then tests, that variable
	{
...
Example: * "Road Test" - Success and failure on the road to nowhere. 
	...
	*: "Road Test"
	...
	Include Unsuccessful Attempt for the Player by Ron Newcomb.
	
	Unsuccessful attempt by an actor doing something: say "Oomph!  [Actor] can't because of the [reason the action failed]."
	
	Unsuccessful attempt by being silly: say "'What are you doing?' says Bob."
	Unsuccessful attempt by Bob being silly: say "'Making yourself comfortable?' you say."
	
	The block jumping rule is not listed in any rulebook.
	The block sleeping rule is not listed in any rulebook.
	[The block waiting rule is not listed in any rulebook.]
	
	Check an actor jumping (this is the easy but pointless rule): say "'No problem.'"; make no decision.
	...
	Carry out an actor jumping: say "[Actor] jumps quietly."
	...
	Test me with "jump / bob, jump / sleep / bob, sleep / wait / bob, wait / take stop sign / bob, take stop sign ".
