Phrases for Tables with Topics by Ron Newcomb begins here.
...
Section 1 - whether (the topic understood) is a topic listed in (table)
...
Section 2 - whether (the player's command) includes (a topic) listed in (table)
...
Section 3 - whether (the player's command) matches (a topic) listed in (table)
...
Section 4 - which i-text is (table column name) corresponding to a topic of (snippet) in (table)
...
Section 5 - say Fix The Topic Understand
...
Example: * An Unexpected Answer - A brief demonstration of power.
