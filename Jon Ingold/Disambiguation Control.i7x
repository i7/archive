...
Messages produced by the parser are stored in the Table of Disambiguation Messages. They can be changed using I7's "amended" syntax (see the Documentation tab). Each message is indexed by a number. The table runs as follows:
...
The "actor or player" phrase is a hook for changing the text into first person, although this is not yet properly supported by this extension or, really, by Inform. Print or Construct is used to either print out the command matched so far, or generate on e of the "lookahead" messages (briefly, these are message 20, 21 and 22, which are formed when the parser plans to use the second noun to work out the first.)

(Thanks to Ron Newcomb for the table-running code.)
